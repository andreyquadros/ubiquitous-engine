...
DIR=/opt/ubiqx/site
NET="${UBIQX_NETWORK:-dokploy-network}"
...
# O nome do certresolver vem do próprio Traefik que já roda aqui; "letsencrypt" é o padrão do Dokploy.
...
docker network inspect "$NET" >/dev/null 2>&1 || {
  echo "a rede $NET não existe nesta VPS: o Dokploy (ou outro Traefik) está de pé?" >&2
  echo "se a rede tiver outro nome, passe UBIQX_NETWORK=<nome>." >&2
  exit 2
}
